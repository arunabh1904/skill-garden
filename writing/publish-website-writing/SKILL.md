...
Read and apply [add-blog-voice](../add-blog-voice/SKILL.md) before generating or validating narration. Its committed narrator anchor and exporter profile are the corpus contract; do not substitute another built-in voice, redesign a speaker per post, or revive sentence-level chunking unless the user explicitly selects a new voice and the complete corpus is revalidated.
...
3. Keep the full article as the default narration. Skip raw markup, tables, captions, references, and visual-only labels only when the prose before and after them carries the complete argument. If the exporter rejects a post for exceeding its duration limit, create a reviewed, source-SHA-pinned narration sidecar that preserves every authored H2 section, governing claim, motive, evidence, and conclusion. Shorten repeated examples; never truncate the MP3 or remove its ending.
4. Run targeted freshness, fixed-narrator-profile, section-coverage, no-anchor-leak, beginning/end completeness, and duration checks. Use ASR as an audit signal for spoken output, not as a replacement for source/digest checks. Then run `git diff --check` and `npm run ci`.
5. Open a second PR containing only the regenerated Blog MP3s, `public/assets/audio/manifest.json`, and any necessary narration sidecar. Include exporter code or tests only when the audio compiler itself had to change.
