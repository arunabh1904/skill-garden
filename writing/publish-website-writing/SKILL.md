...
Read and apply [add-blog-voice](../add-blog-voice/SKILL.md) before generating or validating narration. Its human narrator profile, bounded paragraph-group policy, whole-request decoder, and request-level artifact gate are the release contract. Assign every new or changed Blog post to that profile; do not fall back to the legacy Qwen Base path, redesign a speaker per post, accelerate the finished speech, or weaken the audit thresholds to make a file pass.
...
4. Run targeted freshness, human-profile, source-coverage, beginning/end, duration, waveform-integrity, internal-seam, and request-level ASR checks. Reject multi-word insertions and high-error requests even when whole-file WER is acceptable. Then run `git diff --check` and `npm run ci`.
