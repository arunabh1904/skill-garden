...
description: "Route, format, validate, and ship writing for every authored category of arunabh1904.github.io: Arxiv Notes, Blog, Revision Notes, and Code practice. Use when the user asks to add, publish, or ship website writing. Select one primary category per artifact, load its writing guidance, and ship when the user requests a release."
---
...
Read one primary category skill per artifact; a mixed batch may need several. Each template loads [the universal writing style](../writing-style/SKILL.md):
...
Respect the user's category. Infer only when absent using [the writing route map](../README.md), and report the inference when it matters. Choose by the reader's job, not length or source count: a Blog argument can center on one paper. A multi-paper technical synthesis belongs in Blog.

Separate drafting, site integration, and release. A request to draft, audit, or revise does not by itself authorize shipping or audio generation. For a release request, complete the release workflow below; preserve explicit local-only, text-only, or deferred-audio instructions. For a follow-up to an already-shipped website change, retain the established shipping scope unless the user narrows it.

## Resolve The Paper Radar Queue

Use this section only when the user asks to process Paper Radar actions or its publishing queue. A named paper or an ordinary writing task does not require accessing Gmail or importing queued actions. For a queue task, check `/Users/arunabhmishra/Code/paper-radar` when it exists:
...
3. For a request to ship relevant papers from that queue, treat every queued external paper as candidate scope. Before drafting, compare its `blog_path` with current `origin/main`: if a finished, source-grounded note has already landed, classify it as published and do not regenerate or reship it merely because Paper Radar still says `drafted`. For a queue request naming one paper, keep only that paper in scope unless the user expands it.
...
5. Follow the category skill for titles and headings. During title-only cleanup, preserve the existing `postSlug` and `legacyPath`; title edits do not imply URL migration.
6. Use `$...$` and `$$...$$` for math. Put new local images in `public/assets/images/`.
7. Add only the post file for Arxiv Notes and Blog unless an asset is needed. New Revision Notes and listing metadata changes also require `src/pages/revision_notes.astro`. Code follows its data-file overlay.
8. Leave the homepage untouched unless explicitly requested; collection shelves update automatically.

For a request covering all website writing, inventory every authored section before editing: `paper-shorts`, `blog`, `revision-notes`, and Code practice. For a narrower category or file request, inventory that scope only. Do not treat Arxiv Notes as the complete writing corpus. For callouts specifically, preserve ordinary prose and mark only existing summaries, theses, takeaways, decision rules, or recommendations that already have standalone emphasis value. Confirm that each in-scope category has been reviewed and add a regression check when the convention should persist.
...
2. Inspect the rendered target route when layout, math, images, links, or interactive behavior changed. For Arxiv Notes, enforce [the category image contract](../write-paper-note/references/paper-note-format.md#images-and-captions).
...
4. After the release is live, run the post-ship cleanup below. Preserved unrelated or uncertain work may leave the primary checkout dirty; report that state without treating it as a failed publication.
5. Report inferred category, tags, date, title/slug cleanup, file type, source gaps, and the final primary-checkout status. For paper notes only, also report the inferred `field`, image count, and each image's explanatory job.
...
Read and apply [add-blog-voice](../add-blog-voice/SKILL.md) for the narration profile, source extraction, chunking, and artifact quality contract. This section owns the two-PR order and deployment verification.
...
3. Apply the full-source extraction and duration policy in `add-blog-voice`. Do not abridge the article or create a narration sidecar to make generation fit.
4. Run its freshness, profile, source-coverage, duration, waveform, listening, and request-level ASR checks. Then run `git diff --check` and `npm run ci`.
5. Open a second PR containing the regenerated Blog MP3s and `public/assets/audio/manifest.json`. Include exporter code or tests only when the audio compiler or its full-source extraction contract had to change. Do not add a narration sidecar.
